...
namespace BlogPlatform.Auth.IntegrationTests
{
    public class AuthIntegrationTests : BaseIntegrationTests<AuthApiFactory, Program>
    {
        ...
        public async Task UserRepository_CanCreateAndRetrieveUser()
        {
            ...
            var user = new User(Guid.NewGuid(), "repouser", "repo@example.com", "hashed_password");

            // Act
            var createdUserId = await userRepository.CreateAsync(user);
            var retrievedUser = await userRepository.GetByIdAsync(createdUserId);
