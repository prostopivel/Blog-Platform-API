...
namespace BlogPlatform.Auth.Core.Services
{
    public class AuthService : IAuthService
    {
            ...
            CancellationToken token = default)
        {
            ...
            var createdUserId = await _userRepository.CreateAsync(user, token: token);

            await InvalidateUserCache(createdUserId, user.Email, token: token);

            _logger.LogInformation("User with email {Email} created: {UserId}", email, user.Id);
            return new User(createdUserId, username, email, passwordHash);
        }
